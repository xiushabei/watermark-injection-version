...
### 变更：编辑模式改为手动按键 + 恢复编辑内显隐切换
- **移除聊天栏自动编辑**：放弃 JNI 聊天屏幕检测与光标兼容检测整套机制
  （`JavaDetector` 屏幕检测调用、光标闩锁、失焦去抖全部退役），
  编辑模式**只由按键切换**，不再随聊天栏 / GUI 屏幕自动进出
- **编辑模式按键可配置**：默认 Insert；启动器界面"编辑模式按键 → 修改"捕获
  新按键后写入 `%APPDATA%/WatermarkDLL/edit_key.cfg`（纯文本十进制 VK 码），
  DLL 每 500ms 轮询该文件，改键即时生效（含已注入的游戏进程）
- **恢复显隐切换（限编辑模式）**：编辑模式下**双击贴图**切换隐藏状态；
  隐藏的贴图平时完全跳过渲染，编辑模式下以 30% 透明度显示以便找回；
  `visible` 字段恢复进 `overlays.json` 存取
- **仍然移除的**：主菜单隐藏（贴图始终显示）、按绑定键显隐、F1 隐藏 HUD 门控
...
- 编辑模式由按键切换（默认 Insert，可在启动器修改），不再依赖聊天栏检测
- 编辑模式下双击贴图切换隐藏/显示；隐藏贴图平时不渲染，编辑模式下淡显以便找回
- 贴图始终显示：主菜单/游戏内均渲染，按 F1 不影响
- 配置自动保存到 `%APPDATA%/WatermarkDLL`（含移动模式/路径点/随机方向/显隐），重启自动恢复
...
| 操作 | 说明 |
|------|------|
| 拖放文件 | 添加PNG/JPG/BMP/GIF图像为叠加层 |
| 按编辑模式键（默认 Insert，启动器可改） | 进入 / 退出编辑模式 |
...
编辑模式曾依赖聊天屏幕自动检测（JNI 反射 + 光标兼容双模式，适配全版本），
见上文问题 1/5/14 的修复记录。现按需求改为**纯手动按键切换**，
检测模块已整体停用（代码保留在 `JavaDetector.h/cpp` 中备查）。
